...
class Members_Widget_Login extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( ! is_admin() ) {
		    $logged_in_text = '<div class="logindir">
				<div class="btn-group">
				  <a class="btn btn-warning" href="#"><i class="icon-user"></i> '.apply_filters( 'widget_text', $instance['logged_in_text'] ).'</a><a class="btn btn-warning dropdown-toggle" data-toggle="dropdown" href="#"><span class="icon-caret-down"></span></a>
				  <ul class="dropdown-menu">
				  <li><a href="/wp-admin/"><i class="icon-fixed-width icon-pencil"></i> Backend</a></li>
				    <li><a href="/profile/"><i class="icon-fixed-width icon-user"></i> Profile</a></li>
				    <li><a href="'. wp_logout_url() .'"><i class="icon-fixed-width icon-signout"></i> Logout</a></li>
				  </ul>
				</div>
			</div>';
		  } else { 
		...
		$logged_out_text = '<div class="logindir">' . apply_filters( 'widget_text', $instance['logged_out_text'] ) . '</div>';
		...
		/* If the current user is not logged in. */
			...
			echo '<div class="members-login-form gform_wrapper pull-right">
		<div class="logindir  pull-right">
				<div class="btn-group  pull-right btn-group-login">
				  <a class="btn btn-warning" href="#"><i class="icon-user"></i> Login</a><a class="btn btn-warning dropdown-toggle" data-toggle="dropdown" href="#"><span class="icon-caret-down"></span></a>
				  <ul class="dropdown-menu" style="width: 275px">
				  <li><div class="logindir-inside">' . wp_login_form( $args ) . '</div></li>
				  </ul>
				</div>
			</div>
		</div>';
	...
	function form( $instance ) {
		...
		$defaults = array(
			...
			'value_username' => '',
			'show_avatar' => false,
			'logged_out_text' => esc_html__( '', 'members' ),
			'logged_in_text' => esc_html__( '[em_username]', 'members' )
		);

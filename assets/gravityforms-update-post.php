...
class gform_update_post
{
	...
	public static function ajax_delete_upload()
	{
		...
		// verify nonce
		if (! wp_verify_nonce($options['nonce'], self::$settings['nonce_delete']) )
		{
			die('Are you sure?');
		}
	...
	public static function scripts_and_styles()
	{
		...
		wp_register_script( self::$prefix, self::$settings['dir'] . 'assets/js/scripts.js', array('jquery'), self::$settings['version'] );
		$args = array(
		...
		elseif (! empty($field['inputType']) && 'fileupload' == $field['inputType'] && ! empty($field['defaultValue']) )
		{
			...
			if ( 'image/' == substr($mime, 0, 6) )
			{
				...
				if ( true === $img_editor_test )
				{
					...
					if (! is_wp_error($image_editor) )
					{
						...
						{
							$image_url = str_replace($path_to_file, $url_to_file, $thumbname);
						}
					}
				}
			...
			<div class="<?php echo esc_attr(self::$prefix); ?>_upload_container">
				...
				<?php if ( apply_filters( self::$prefix . '/public_file_delete', true ) ) : ?>
					<a class="<?php echo esc_attr(self::$prefix); ?>_delete_link" data-post_id="<?php echo esc_attr(self::$post->ID); ?>" data-form_id="<?php echo esc_attr($form_id); ?>" data-meta="<?php echo esc_attr($field['postCustomFieldName']); ?>" href="#<?php _e("delete_requires_javascript", self::$prefix); ?>" title="<?php _e("Delete Upload", self::$prefix); ?>">
	...
	public static function gform_pre_render( $form )
	{
		if ( self::current_user_can() )
		{
			...
			foreach ( $form['fields'] as &$field )
			{
				...
				{
					$multi_fields = array('multiselect', 'checkbox');
	 ...
	 *
	 * While nice, custom post types only adds taxonomies on intead of updating the whole set, so you can't ever remove them. This wipes them out and then they can be added through the custom post types plugin.
	 *
	 * @author  Jake Snyder
	 * @return	void
	 */
...
if (! function_exists('gravityforms_update_post') )
{
	/**
	 * Gateway Function (deprecated)
	 *
	 * This wraps the class and allows access to its methods. This may be removed in future releases. It is only here to support early adopters of the first replacement function I wrote. The preferred method is just to use the class to access its static methods.
	 * -Jake
	 *
	 * @param $method Method name you want to use
	 * @param $args Arguments to pass to the method
	 * @return mixed
	 */
	function gravityforms_update_post( $method='setup_form', $post_id=false )
	{
		if (! $post_id && ! empty($GLOBAL))
		if ( 'setup_form' == $method && is_numeric($post_id) )
		{
